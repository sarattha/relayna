    ...
    async def replay(
        ...
        *,
        ...
        try:
            ...
            consume_args = topology.status_stream_consume_arguments()
            if isinstance(start_offset, str):
                consume_args["x-stream-offset"] = start_offset
            else:
                consume_args["x-stream-offset"] = int(start_offset)

            iterator_timeout = max(self._idle_timeout_seconds, 0.01)
            if max_seconds is not None:
                iterator_timeout = min(iterator_timeout, max(max_seconds, 0.01))

            async with queue.iterator(arguments=consume_args or None, timeout=iterator_timeout) as iterator:
...
__all__ = ["StreamHistoryReader", "StreamOffset"]
