...
ROUTE_SOURCE_FILES = sorted((ROOT / "studio" / "backend" / "src" / "relayna_studio").glob("*.py"))
...
def test_studio_backend_route_declarations_match_production_freeze_manifest() -> None:
    manifest = json.loads((FREEZE_DIR / "routes.json").read_text())
    expected = manifest["route_declarations"]
